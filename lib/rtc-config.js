...
function summarizeRtcConfig(config) {
  const protocols = new Set();
  for (const server of config.iceServers || []) {
    const urls = Array.isArray(server.urls) ? server.urls : [server.urls];
    for (const url of urls) {
      const protocol = String(url).split(":")[0];
      if (["stun", "turn", "turns"].includes(protocol)) protocols.add(protocol);
    }
  }
  return {
    relayConfigured: config.relayConfigured === true,
    serverCount: Array.isArray(config.iceServers) ? config.iceServers.length : 0,
    protocols: [...protocols].sort(),
  };
}

async function rtcStatusHandler(request, response) {
  response.set("Cache-Control", "private, no-store");
  if (request.get("sec-fetch-site") === "cross-site") {
    return response.status(403).json({ error: "CROSS_SITE_REQUEST" });
  }
  try {
    response.json(summarizeRtcConfig(await createRtcConfig()));
  } catch {
    console.error("[younme:rtc] TURN_STATUS_UNAVAILABLE");
    response.status(503).json({ error: "TURN_STATUS_UNAVAILABLE" });
  }
}
